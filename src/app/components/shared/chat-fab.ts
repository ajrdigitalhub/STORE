  ...
  template: `
    <div class="fixed bottom-6 right-6 z-[9999] flex flex-col items-end gap-4">
      ...
      <div class="relative">
        ...
        <!-- Chat Window -->
        @if (isOpen()) {
          <div class="absolute bottom-20 right-0 w-[350px] h-[500px] bg-metallic-black border border-white/10 rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
            <!-- Header -->
            <div class="p-6 bg-accent/5 border-b border-white/5 flex items-center gap-4">
              ...
              <div>
                <h3 class="font-bold text-sm">IDEA Zone Support</h3>
                <p class="text-[10px] text-accent-muted uppercase tracking-widest">Online & Ready to Help</p>
              </div>
            </div>
            ...
            <div #scrollContainer class="flex-grow overflow-y-auto p-6 space-y-4 no-scrollbar">
              <div class="flex flex-col gap-1">
                <div class="bg-white/5 text-accent-muted text-[10px] px-3 py-1 rounded-full self-center mb-4">
                  Today
                </div>
              </div>
...
export class ChatFabComponent implements AfterViewChecked {
  ...
  quickActions: QuickAction[] = [
    ...
    { label: 'Product Inquiry', icon: 'inventory_2', message: 'I have a question about a product.' },
    { label: 'Technical Support', icon: 'build', message: 'I need technical assistance with my print.' },
    { label: 'Bulk Order Pricing', icon: 'payments', message: 'I want to inquire about bulk order discounts.' }
  ];
  ...
  sendMessage() {
    if (this.messageText.trim()) {
      this.userHasSentMessage.set(true);
